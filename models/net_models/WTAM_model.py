...
from models.net_models.base_model import BaseModel
import time
...
class WTAMModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.mask_global = torch.ByteTensor(self.opt.batchSize, 1, opt.fineSize, opt.fineSize)
        ...
        self.netG = networks.define_G(input_nc, opt.output_nc, opt.ngf,
                                      opt.which_model_netG, opt.init_type, self.gpu_ids, opt.init_gain)
        ...
        if self.isTrain:
            ...
            self.netD = networks.define_D(opt.input_nc, opt.ndf,
                                          ...
                                          opt.n_layers_D, opt.norm, use_sigmoid, opt.use_spectral_norm_D, opt.init_type,
        ...
        if self.isTrain:
            ...
            self.criterionL1_mask = networks.Discounted_L1(opt).to(
                self.device)  # make weights/buffers transfer to the correct device
            # VGG loss
    ...
    def set_input(self, input):
        ...
        if not self.opt.offline_loading_mask:
            if self.opt.mask_type == 'center':
                ...
                int(self.opt.fineSize / 4) + self.opt.overlap: int(self.opt.fineSize / 2) + int(
                    self.opt.fineSize / 4) - self.opt.overlap] = 1
                self.rand_t, self.rand_l = int(self.opt.fineSize / 4) + self.opt.overlap, int(
                    self.opt.fineSize / 4) + self.opt.overlap
            elif self.opt.mask_type == 'random':
                self.mask_global = self.create_random_mask().type_as(self.mask_global).view(1, *self.mask_global.size()[
                                                                                                -3:])
        ...
        if self.opt.add_mask2input:
            ...
            real_A = torch.cat((real_A, (1 - self.mask_global).expand(real_A.size(0), 1, real_A.size(2),
    ...
    def set_gt_latent(self):
        if self.opt.add_mask2input:
            # make it 4 dimensions.
            # Mention: the extra dim, the masked part is filled with 0, non-mask part is filled with 1.
            real_B = torch.cat([self.real_B, (1 - self.mask_global).expand(self.real_B.size(0), 1, self.real_B.size(2),
                                                                           self.real_B.size(3)).type_as(self.real_B)],
                               dim=1)
        else:
            real_B = self.real_B
    ...
    def backward_G(self):
        ...
        for i in range(3):
            self.loss_style += self.criterionL2_style_loss(util.gram_matrix(vgg_ft_fakeB[i]),
